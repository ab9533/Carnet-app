...
// (Console Firebase → Paramètres du projet → Vos applications → config de l'app Web)
...
function translateAuthError(err) {
  ...
  return map[err && err.code] || "Une erreur est survenue. Réessaie.";
}
...
function saveState() {
  ...
  cloudSaveTimer = setTimeout(() => {
    setDoc(doc(db, 'plans', currentUser.uid), state).catch(() => { /* pas de réseau — réessaiera au prochain changement */ });
  }, 600);
}

let state = loadState();
let ui = { tab: 'today', date: new Date(), scheduling: null, quickOpen: false, ritualForm: null, noteTag: 'Toutes', noteEditing: null, describeOpen: false, describeDate: null, parsedItems: null, authMode: 'login', authError: null };
...
function renderLogin(mode, errorMsg) {
  ...
  return `
  <div style="max-width:340px;margin:15vh auto 0;padding:0 16px;">
    <div class="header-date" style="text-align:center;margin-bottom:4px;">Carnet</div>
    <div style="text-align:center;color:#9CA3AF;font-size:13px;margin-bottom:20px;">${isSignup ? 'Créer un compte' : 'Se connecter'}</div>
    <div class="card">
      <input id="auth-email" class="input" type="email" placeholder="Email" style="margin-bottom:8px;" autocomplete="username" />
      <input id="auth-password" class="input" type="password" placeholder="Mot de passe" style="margin-bottom:8px;" autocomplete="${isSignup ? 'new-password' : 'current-password'}" />
      ${errorMsg ? `<div style="color:#A6635A;font-size:12px;margin-bottom:8px;">${esc(errorMsg)}</div>` : ''}
      <button class="icon-btn" style="background:#23252B;color:#fff;border:none;width:100%;margin-bottom:8px;" data-action="${isSignup ? 'do-signup' : 'do-login'}">${isSignup ? 'Créer le compte' : 'Se connecter'}</button>
      <button class="today-link" style="width:100%;text-align:center;" data-action="${isSignup ? 'go-login' : 'go-signup'}">${isSignup ? 'Déjà un compte ? Se connecter' : "Pas encore de compte ? En créer un"}</button>
    </div>
  </div>`;
}
...
/* ---------- render ---------- */
function render() {
  if (!currentUser) { root.innerHTML = renderLogin(ui.authMode, ui.authError); return; }
...
function renderHeader(isToday) {
  ...
  <div style="margin-bottom:20px;">
    <div style="display:flex;justify-content:flex-end;margin-bottom:2px;">
      <button class="today-link" style="margin:0;" data-action="sign-out">déconnexion</button>
    </div>
...
function handleClick(e) {
  ...
  if (action === 'go-signup') { ui.authMode = 'signup'; ui.authError = null; render(); return; }
  if (action === 'go-login') { ui.authMode = 'login'; ui.authError = null; render(); return; }
  if (action === 'do-login') {
    const email = document.getElementById('auth-email').value.trim();
    const pass = document.getElementById('auth-password').value;
    signInWithEmailAndPassword(auth, email, pass).catch(err => { ui.authError = translateAuthError(err); render(); });
    return;
  }
  if (action === 'do-signup') {
    const email = document.getElementById('auth-email').value.trim();
    const pass = document.getElementById('auth-password').value;
    createUserWithEmailAndPassword(auth, email, pass).catch(err => { ui.authError = translateAuthError(err); render(); });
    return;
  }
  if (action === 'sign-out') { signOut(auth); return; }
...
function handleKeydown(e) {
  ...
  if (e.target.id === 'auth-email' || e.target.id === 'auth-password') {
    document.querySelector('[data-action="do-login"], [data-action="do-signup"]')?.click();
  }
}
...
/* ---------- démarrage : authentification puis synchro cloud ---------- */
onAuthStateChanged(auth, async (user) => {
  if (cloudUnsub) { cloudUnsub(); cloudUnsub = null; }
  currentUser = user;
  if (!user) { render(); return; }

  state = loadState(); // copie locale immédiate, pour un affichage instantané
  render();

  const ref = doc(db, 'plans', user.uid);
  try {
    const snap = await getDoc(ref);
    if (!snap.exists()) await setDoc(ref, state); // premier login : on pousse la copie locale existante
  } catch (e) { /* pas de réseau — on reste sur la copie locale pour l'instant */ }

  cloudUnsub = onSnapshot(ref, (snap) => {
    if (!snap.exists()) return;
    state = { ...EMPTY, ...snap.data() };
    saveLocalOnly();
    render();
  }, () => { /* synchro interrompue — la copie locale reste utilisable hors-ligne */ });
});
